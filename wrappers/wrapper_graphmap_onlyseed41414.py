...
ALIGNER_PATH = SCRIPT_PATH + '/../aligners/graphmap-onlyseed41414/graphmap/';
...
def download_and_install():
	...
	command = 'cd %s; mkdir graphmap-onlyseed41414; cd graphmap-onlyseed41414; git clone %s' % (basicdefines.ALIGNERS_PATH_ROOT_ABS, ALIGNER_URL);
	subprocess.call(command, shell='True');
	sys.stderr.write('\n');
	...
	sys.stderr.write('[%s wrapper] Checking out branch fixedseed.\n' % (MAPPER_NAME));
	command = 'cd %s; git checkout onlyseed41414' % (ALIGNER_PATH);
	subprocess.call(command, shell='True');
	sys.stderr.write('\n');
