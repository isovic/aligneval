...
ALIGNER_PATH = SCRIPT_PATH + '/../aligners/graphmap-noL1/graphmap/';
...
def download_and_install():
	...
	command = 'cd %s; mkdir graphmap-noL1; cd graphmap-noL1; git clone %s' % (basicdefines.ALIGNERS_PATH_ROOT_ABS, ALIGNER_URL);
	subprocess.call(command, shell='True');
	sys.stderr.write('\n');
	...
	sys.stderr.write('[%s wrapper] Checking out branch noL1.\n' % (MAPPER_NAME));
	command = 'cd %s; git checkout noL1' % (ALIGNER_PATH);
	subprocess.call(command, shell='True');
	sys.stderr.write('\n');
