...
ALIGNER_PATH = SCRIPT_PATH + '/../aligners/graphmap-noLCSkButWithL1/graphmap/';
...
def download_and_install():
	...
	command = 'cd %s; mkdir graphmap-noLCSkButWithL1; cd graphmap-noLCSkButWithL1; git clone %s' % (basicdefines.ALIGNERS_PATH_ROOT_ABS, ALIGNER_URL);
	subprocess.call(command, shell='True');
	sys.stderr.write('\n');
	...
	sys.stderr.write('[%s wrapper] Checking out branch noLCSkButWithL1.\n' % (MAPPER_NAME));
	command = 'cd %s; git checkout noLCSkButWithL1' % (ALIGNER_PATH);
	subprocess.call(command, shell='True');
	sys.stderr.write('\n');
