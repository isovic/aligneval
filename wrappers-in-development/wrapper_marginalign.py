...
ALIGNER_PATH = SCRIPT_PATH + '/../aligners/marginAlign';
BIN = 'marginAlign';
...
def run(reads_file, reference_file, machine_name, output_path, output_suffix=''):
	...
	if ((machine_name.lower() == 'illumina') or (machine_name.lower() == 'roche')):
		job_tree = 'jobTree-%s' % (machine_name.lower());
		parameters = '--jobTree %s/%s --em --maxThreads=%d --logInfo --defaultMemory=100000000000 --defaultCpu=%d' % (ALIGNER_PATH, job_tree, num_threads, num_threads);
	...
	command = '%s %s/%s %s %s %s > %s' % (basicdefines.measure_command(memtime_file), ALIGNER_PATH, BIN, reads_file, reference_file. sam_file, parameters);
	sys.stderr.write('[%s wrapper] %s\n' % (MAPPER_NAME, command));
	subprocess.call(command, shell=True);
	sys.stderr.write('\n\n');
